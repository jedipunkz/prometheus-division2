...
def main():
    users = ['jedipigpig', 'gatchaman.jp', 'lobelia_dixon', 'hayate_ewing', 'souyuh.jp']
    vars = ['timeplayed_total', 'timeplayed_dz', 'timeplayed_pve', 'timeplayed_pvp', 'timeplayed_rogue', 'level_pve', 'level_dz', 'kills_npc', 'kills_pvp', 'kills_pve_hyenas', 'kills_pve_outcasts', 'kills_pve_blacktusk', 'kills_pve_truesons', 'kills_pve_dz_hyenas', 'kills_pve_dz_outcasts', 'kills_pve_dz_blacktusk', 'kills_pve_dz_truesons']
    dict = {}
    for user in users:
        for var in vars:
            dict[user + '_' + var] = Gauge(user.replace('.', '_') + '_' + var, 'Gauge')
